...
def calcCYK(w):
    ...
    # initialization: 
    for i in xrange(L):
        for pdrow in production:
            if (pdrow[2]==-1 and pdrow[1]==w[i]):
                X[i][i][pdrow[0]]=True

    for t in xrange(2, L+1):
        for i in xrange(0, L-t+1):
            j = i+t-1
            # print i, j, X[i][j]
            for p in xrange(i, j):
                #print '\t', p, '-->', i, p, p+1, j, X[i][p], X[p+1][j]
                for Ai, Am in enumerate(X[i][p]):
                    for Bi, Bm in enumerate(X[p+1][j]):
                        if Am and Bm:
                            #print '\t updates', Ai, Bi, Am, Bm
                            for pdrow in production:
                                if [Ai, Bi] == pdrow[1:3]: 
                                    #print '-----> found:', [Ai, Bi]
                                    X[i][j][pdrow[0]]=True
